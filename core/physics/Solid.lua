...
function Solid:initialize( mass, position, rotation, collisionShape )
    ...
    self.handle = Scheduler.awaitCall(engine.CreateSolid,
        mass,
        position[1],
        position[2],
        position[3],
...
function Solid:destroy()
    ...
    Scheduler.blindCall(engine.DestroySolid, self.handle)
    self.handle = nil
end

--- Returns the solids mass or zero, if it's static.
function Solid:getMass()
    return Scheduler.awaitCall(engine.GetSolidMass, self.handle)
end
...
function Solid:setMass( mass )
    assert(mass >= 0, 'Mass must be positive.')
    Scheduler.blindCall(engine.SetSolidMass, self.handle, mass)
end
...
function Solid:setRestitution( restitution )
    ...
    Scheduler.blindCall(engine.SetSolidRestitution, self.handle, restitution)
end

--- Changes a solids friction.
function Solid:setFriction( friction )
    assert(friction >= 0, 'Friction must be positive.')
    Scheduler.blindCall(engine.SetSolidFriction, self.handle, friction)
end
...
function Solid:setCollisionThreshold( threshold )
    ...
    Scheduler.blindCall(engine.SetSolidCollisionThreshold, self.handle, threshold)
end
...
function Solid:disableCollisionEvents()
    Scheduler.blindCall(engine.SetSolidCollisionThreshold, self.handle, -1)
end

--- Returns the solids current position as 3d vector.
function Solid:getPosition()
    return Vec(Scheduler.awaitCall(engine.GetSolidPosition, self.handle))
end

--- Returns the solids current orientation as quaternion.
function Solid:getRotation()
    return Quat(Scheduler.awaitCall(engine.GetSolidRotation, self.handle))
end

--- Velocity at which the solid moves through space.
-- @return[type=core.Vector]
function Solid:getLinearVelocity()
    return Vec(Scheduler.awaitCall(engine.GetSolidLinearVelocity, self.handle))
end

--- Velocity at which the solid rotates around itself.
-- @return[type=core.Vector]
function Solid:getAngularVelocity()
    return Vec(Scheduler.awaitCall(engine.GetSolidAngularVelocity, self.handle))
end
...
function Solid:enableGravity( enable )
    Scheduler.blindCall(engine.EnableGravityForSolid, self.handle, enable)
end
...
function Solid:applyImpulse( value, relativePosition, useLocalCoordinates )
    ...
    Scheduler.blindCall(engine.ApplySolidImpulse,
        self.handle,
        value[1],
        value[2],
        value[3],
...
function Solid:createForce()
    local force = Force(Scheduler.awaitCall(engine.CreateForce, self.handle))
...
Scheduler.blindCall(engine.SetEventCallback, 'Collision', CollisionHandler)
