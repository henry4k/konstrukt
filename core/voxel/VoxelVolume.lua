...
function VoxelVolume:initialize( size )
    ...
    self.handle = Scheduler.awaitCall(engine.CreateVoxelVolume, size:unpack(3))
...
function VoxelVolume:destroy()
    ...
    Scheduler.blindCall(engine.DestroyVoxelVolume, self.handle)
    self.handle = nil
end
...
function VoxelVolume:_readVoxelData( position )
    ...
    local voxelData = Scheduler.awaitCall(engine.ReadVoxelData, self.handle, position:unpack(3))
    if voxelData then
...
function VoxelVolume:_writeVoxelData( position, voxelData )
    ...
    local result = Scheduler.awaitCall(engine.WriteVoxelData, self.handle, position[1], position[2], position[3], voxelData)
    if result then
